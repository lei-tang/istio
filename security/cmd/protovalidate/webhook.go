...
// Webhook implements a validating webhook for demo webhook validation.
type Webhook struct {
	mu                     sync.RWMutex
...
// WebhookParameters configures parameters for the webhook.
...
// NewWebhook creates a new instance of a validating webhook.
func NewWebhook(p WebhookParameters) (*Webhook, error) {
	// TODO: protovalidate waits for Chiron to provision the webhook certificate through the secret mount.
	// The secret "istio.istio-protovalidate-service-account"
...
func (wh *Webhook) validate(ar *v1beta1.AdmissionReview) *v1beta1.AdmissionResponse {
	...
	log.Infof("available labels: %v", pod.Labels)
	log.Infof("required labels: %v", requiredLabels)
	// If the pod label does not contain all required labels, reject
	for _, rl := range requiredLabels {
		if _, ok := pod.Labels[rl]; !ok {
			allowed = false
			rt = &metav1.Status{
...
func (wh *Webhook) serveValidate(w http.ResponseWriter, r *http.Request) {
	log.Debugf("enter serveValidate(), request: %v", r)
