...
var (
	// Service accounts for the webhooks.
	...
	// The service names of the webhooks. Each item corresponds to an item at the same index in WebhookServiceAccounts.
	...
	// The service ports of the webhooks. Each item corresponds to an item at the same index in WebhookServiceNames.
...
func (wc *WebhookController) GenKeyCertK8sCA(saName string, saNamespace string) ([]byte, []byte, error) {
	...
	if err != nil {
		...
		wc.cleanUpCertGen(csrName, csrCreated)
		return nil, nil, err
	}
	...
	// 5. Clean up the artifacts (e.g., delete CSR)
	err = wc.cleanUpCertGen(csrName, csrCreated)
	return certChain, keyPEM, nil
}

// Clean up the CSR
func (wc *WebhookController) cleanUpCertGen(csrName string, csrCreated bool) error {
	if csrCreated {
		// Delete CSR
