...
func OutputKeyCertToDir(dir string, priviateKey, certChain, rootCert []byte) error {
	if len(dir) == 0 {
		return nil
	}
	if priviateKey == nil || certChain == nil || rootCert == nil {
		return fmt.Errorf("nil detected for the input private key, cert chain, or root cert")
	}
	if err := ioutil.WriteFile(path.Join(dir, "key.pem"), priviateKey, 0700); err != nil {
		return fmt.Errorf("error when writing the private key: %v", err)
	}
	if err := ioutil.WriteFile(path.Join(dir, "cert-chain.pem"), certChain, 0700); err != nil {
		return fmt.Errorf("error when writing the cert chain: %v", err)
	}
	if err := ioutil.WriteFile(path.Join(dir, "root-cert.pem"), rootCert, 0700); err != nil {
		return fmt.Errorf("error when writing the root cert: %v", err)
	}
	return nil
}
