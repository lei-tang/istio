...
docker.vault-tls-basic-server:  | $(ISTIO_DOCKER)
	cp security/docker/Dockerfile.vault-tls-basic-server security/docker/vault-tls-config.hcl $(ISTIO_DOCKER)/ && \
	cp security/docker/vault-tls-cert.pem security/docker/vault-tls-key.pem $(ISTIO_DOCKER)/ && \
	cd $(ISTIO_DOCKER) && \
	docker build -t $(HUB)/vault-tls-basic-server:$(TAG) -f Dockerfile.vault-tls-basic-server . && \
	docker push $(HUB)/vault-tls-basic-server:$(TAG)
